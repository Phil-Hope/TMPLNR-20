...
@Component({
  selector: 'app-list-pending-secondary-shifts',
  templateUrl: './list-pending-secondary-shifts.page.html',
  styleUrls: ['./list-pending-secondary-shifts.page.scss'],
})
export class ListPendingSecondaryShiftsPage implements OnInit {
        ...
        tap(async (res) => {
            if (!(res instanceof ShiftTrackerError) && res.length === 0) {
              const alert = await this.alertCtrl.create({
                ...
                message: 'No Pending "Secondary" Level shifts were found in the database.',
                buttons: ['OK'] });
              await alert.present();
            }
          }
